...
onMounted(() => {
   const xxxxx = useSearchChampions()
   ...
   step.value = useSearchDropdown(step, filteredChamps, input, inputRef, championsRef)
})
         ...
         <div class="nav-search" :class="{ focus: store.navContainerFocus }">
            <div>
               <input ref="inputBind" type="text" spellcheck="false" autocomplete="off"
                  @focus="store.navContainerFocus = true" @keyup.enter="summonerSearch"
                  @keyup.esc="blurInput()"  v-model="input">
